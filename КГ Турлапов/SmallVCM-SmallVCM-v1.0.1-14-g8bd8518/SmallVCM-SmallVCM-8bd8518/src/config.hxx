...
struct Config
{
    ...
    {
        static const char* algorithmNames[1] =
        {
            "light tracing"
        };
...
void ParseCommandline(int argc, const char *argv[], Config &oConfig)
{
	...
	oConfig.mLightIntensity = 200.f;				// [cmd]
    ...
    for(int i=1; i<argc; i++)
    {
		...
		else if(arg == "-l") // light coef
        {
            ...
            std::istringstream iss(argv[i]);
			iss >> oConfig.mLightIntensity;

			if(iss.fail() || oConfig.mLightIntensity < 0)
            {
                printf("Invalid <light coef> argument, please see help (-h)\n");
                return;
            }
        }
    ...
    if(oConfig.mAlgorithm == Config::kAlgorithmMax)
    {
        oConfig.mAlgorithm = Config::kLightTracing;
    }
