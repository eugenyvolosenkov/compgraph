...
public:
    ...
    void Setup(
        const Vec3f &aPosition,		// Координаты
        const Vec3f &aForward,		// Направление
        const Vec3f &aUp,			// Направление вверх
        const Vec2f &aResolution,	// Разрешение камеры
        float       aHorizontalFOV)	// Угол обзора
    {
...
public:
    ...
    Vec3f mForward;
    Vec2f mResolution;
    Mat4f mRasterToWorld;	// М для преобразования координат
    Mat4f mWorldToRaster;	// М для преобразования координат
    float mImagePlaneDist;	// Расстояние (камера - бмп)
};
